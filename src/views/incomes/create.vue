...
<template>
    ...
    <CCard>
        ...
        <CCardHeader component="h5">Create Income</CCardHeader>
        <CCardBody>
            <CForm>
                <CRow>
                    <CCol xs="12" sm="6">
  ...
  export default {
    ...
    methods:{
        save(){
            if(this.name)
            {
                let url = 'http://localhost/financial/api/incomes/create.php';
                axios.post(url,JSON.stringify({name:this.name,value:this.value,date:this.date})).then(res => {
